...
import { customeGetAuctionMetaByStatus } from '../../../apiContracts/listenAuction'
import { AuctionStatus } from '../../../apiContracts/constants'
...
const ListCompletedAuction = () => {
    ...
    useEffect(() => {
        setLoading(true)
        loadNFTTypes();
        customeGetAuctionMetaByStatus(AuctionStatus.complete, 0).then(auctions => {
            auctions?.sort((a, b) => parseInt(b?.auctionID) - parseInt(a?.auctionID))
            setAuctions(auctions)
            const offset = (currentPage - 1) * pageSize
            const limit = pageSize
            const pageNFTs = auctions.slice(offset, limit)
...
export default ListCompletedAuction;